...
# PolyPVT模型的测试
def test_single_volume_PolyPVT(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None,
                               z_spacing=1):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    if len(image.shape) == 3:
        prediction = np.zeros_like(label)
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():
                P1, P2 = net(input)
                outputs = F.upsample(P1 + P2, slice.shape[:], mode='bilinear', align_corners=False)
                # outputs = F.interpolate(outputs, size=slice.shape[:], mode='bilinear', align_corners=False)
                out = torch.softmax(outputs, dim=1)
                out = torch.argmax(out, dim=1).squeeze(0)
                out = out.cpu().detach().numpy()
...
def one_hot(num_class, target, shape):
