...
from PvtMissFormer_lzh.linear_attention import *
from PvtMissFormer_lzh.parc import *
...
class EfficientSelfAtten(nn.Module):
    def __init__(self, dim, head, reduction_ratio):
        ...
        # self.pos = PosCNN(dim, dim, 1)

        if reduction_ratio > 1:
            self.sr = nn.Conv2d(dim, dim, reduction_ratio, reduction_ratio)
            self.norm = nn.LayerNorm(dim)

    def forward(self, x: torch.Tensor, H=7, W=7) -> torch.Tensor:
        # x = self.pos(x)
        B, N, C = x.shape
...
class EfficientSelfAtten_parc(nn.Module):
    def __init__(self, dim, head, reduction_ratio):
        super().__init__()
        self.head = head
        self.reduction_ratio = reduction_ratio
...
class MixFFN_skip_1(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # print("add_1",self.norm1(self.dwconv(self.fc1(x), H, W)).shape)
        # print("add_2",self.fc1(x).shape)
        ax = self.act(self.norm1(self.dwconv(self.fc1(x), H, W) + self.fc1(x)))
        out = self.fc2(ax)
        # print("fc2", out.shape)
        return out
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, N, C = x.shape
        tx = x + self.attn(self.norm1(x), int(N ** 0.5), int(N ** 0.5))
        mx = tx + self.mlp(self.norm2(tx), int(N ** 0.5), int(N ** 0.5))
        return mx


class Rel_TransformerBlock(nn.Module):
    def __init__(self, dim, head, reduction_ratio, token_mlp='mix'):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = LinearAttention(dim, head, reduce_size=reduction_ratio)
        # print(self.attn)
        self.norm2 = nn.LayerNorm(dim)
        if token_mlp == 'mix':
            self.mlp = MixFFN(dim, int(dim * 4))
        elif token_mlp == 'mix_skip':
            self.mlp = MixFFN_skip_2(dim, int(dim * 4))
        else:
            self.mlp = MLP_FFN(dim, int(dim * 4))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # print(x.shape)
        B, N, C = x.shape
        x = x.reshape(B, C, int(N ** 0.5), int(N ** 0.5))
        # print("rel_input",x.shape)
        tx = x + self.attn(x)[0]
        mx = self.mlp(tx)
        tx = tx.reshape(B, -1, C)
        mx = tx + mx
...
class Rel_TransformerBlock_1(nn.Module):
    def __init__(self, dim, head, reduction_ratio, token_mlp='mix'):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = LinearAttention_expand(dim, head, reduce_size=reduction_ratio)
        # print(self.attn)
        self.norm2 = nn.LayerNorm(dim)
        if token_mlp == 'mix':
            self.mlp = MixFFN(dim, int(dim * 4))
...
class Rel_TransformerBlock_1_APE(nn.Module):
    def __init__(self, dim, head, reduction_ratio, APE_shape, token_mlp='mix'):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = LinearAttention_expand_APE(dim, APE_shape, head, reduce_size=reduction_ratio)
        # print(self.attn)
        self.norm2 = nn.LayerNorm(dim)
        if token_mlp == 'mix':
            self.mlp = MixFFN(dim, int(dim * 4))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        mx = tx + mx
        return mx
class Rel_TransformerBlock_1_rel_SAM_Swin(nn.Module):
    def __init__(self, dim, head, reduction_ratio, swin_window_size, token_mlp='mix'):
        super().__init__()
        self.swin_window_size = swin_window_size
        self.norm1 = nn.LayerNorm(dim)
        self.attn = LinearAttention_expand_rel_SAM_Swin(dim,swin_window_size, head, reduce_size=reduction_ratio)
        # print(self.attn)
        self.norm2 = nn.LayerNorm(dim)
        if token_mlp == 'mix':
            self.mlp = MixFFN(dim, int(dim * 4))
...
class Rel_TransformerBlock_1_rel_SAM(nn.Module):
    def __init__(self, dim, head, reduction_ratio, swin_window_size, token_mlp='mix'):
        super().__init__()
        self.swin_window_size = swin_window_size
        self.norm1 = nn.LayerNorm(dim)
        self.attn = LinearAttention_expand_rel_SAM(dim, swin_window_size, head, reduce_size=reduction_ratio)
        # print(self.attn)
        self.norm2 = nn.LayerNorm(dim)
        if token_mlp == 'mix':
            self.mlp = MixFFN(dim, int(dim * 4))
...
class Pos_emb_TransformerBlock(nn.Module):
    def __init__(self, dim, head, reduction_ratio, token_mlp='mix'):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = LinearAttention_pos_1(dim, head, reduce_size=reduction_ratio)
        # print(self.attn)
        self.norm2 = nn.LayerNorm(dim)
        if token_mlp == 'mix':
            self.mlp = MixFFN(dim, int(dim * 4))
...
class WithoutRel_TransformerBlock(nn.Module):
    def __init__(self, dim, head, reduction_ratio, token_mlp='mix'):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = LinearAttention_1(dim, head, reduce_size=reduction_ratio)
        # print(self.attn)
        self.norm2 = nn.LayerNorm(dim)
        if token_mlp == 'mix':
            self.mlp = MixFFN(dim, int(dim * 4))
...
class Pos_TransformerBlock(nn.Module):
    def __init__(self, dim, head, reduction_ratio, token_mlp='mix'):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = LinearAttention_2(dim, head, reduce_size=reduction_ratio)
        # print(self.attn)
        self.norm2 = nn.LayerNorm(dim)
        if token_mlp == 'mix':
            self.mlp = MixFFN(dim, int(dim * 4))
...
class Spe_TransformerBlock(nn.Module):
    def __init__(self, dim, head, reduction_ratio, token_mlp='mix'):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = LinearAttention_Spe(dim, head, reduce_size=reduction_ratio, rel_pos=False)
        # print(self.attn)
        self.norm2 = nn.LayerNorm(dim)
        if token_mlp == 'mix':
            self.mlp = MixFFN(dim, int(dim * 4))
    ...
    def forward(self, x: torch.Tensor, out_high) -> torch.Tensor:
        # print(x.shape)
        # x = self.norm1(x)
        # print(x.shape)
        B, N, C = x.shape
...
class Rel_Spe_TransformerBlock(nn.Module):
    def __init__(self, dim, head, reduction_ratio, token_mlp='mix'):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = LinearAttention_Spe_2(dim, head, reduce_size=reduction_ratio, rel_pos=False)
        # print(self.attn)
        self.norm2 = nn.LayerNorm(dim)
        if token_mlp == 'mix':
            self.mlp = MixFFN(dim, int(dim * 4))
...
class TransformerBlock_parc(nn.Module):
    def __init__(self, dim, head, reduction_ratio=1, token_mlp='mix'):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = EfficientSelfAtten(dim, head, reduction_ratio)
...
if __name__ == '__main__':
    input = torch.randn(24, 49, 512)
    # se = EfficientSelfAtten(512, 8, 4)
    # se = Efficient_Cls_SelfAtten(512, 9, 8, 4)
    se = EfficientSelfAtten(512, 4, 4)
    # se = TransformerBlock(512, 8, 4)
    output = se(input)
    print(output.shape)
